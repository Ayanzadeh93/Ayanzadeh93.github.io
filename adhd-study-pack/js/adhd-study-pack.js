...
function rememberComfort(settings) {
  try { localStorage.setItem(COMFORT_KEY, JSON.stringify({ comfort:settings.comfort, theme:settings.theme, accent:settings.accent, accentGradient:settings.accentGradient })); } catch (e) {}
}

const ACCENT_PALETTE = [
  { id:'focus', name:'Ember', sw:'#CC4520', group:'classic' },
  { id:'rest', name:'Moss', sw:'#0A7F5F', group:'classic' },
  { id:'long', name:'Iris', sw:'#5B4FD1', group:'classic' },
  { id:'alert', name:'Amber', sw:'#9A5F00', group:'classic' },
  { id:'slate', name:'Slate', sw:'#475569', group:'pro' },
  { id:'navy', name:'Navy', sw:'#1E3A5F', group:'pro' },
  { id:'teal', name:'Teal', sw:'#0F766E', group:'pro' },
  { id:'sage', name:'Sage', sw:'#4F6F52', group:'pro' },
  { id:'graphite', name:'Graphite', sw:'#3D4450', group:'pro' }
];
const ACCENT_IDS = ACCENT_PALETTE.map(a => a.id);
const GRADIENT_IDS = ['off', 'soft', 'rich'];
...
const DEFAULTS = () => ({
  ...
  settings: Object.assign({ focus:25, short:5, long:15, cycles:4, autoBreak:true, autoFocus:false, titleClock:true,
              ...
              theme:deviceRecord().theme || 'auto', accent:deviceRecord().accent || 'focus',
              accentGradient:deviceRecord().accentGradient || 'off',
...
function migrate(raw) {
  ...
  out.passport = normalisePassport(raw.passport);
  ...
  try {
    ...
    if (raw) {
      ...
      if (S.meta.sample) {
        ...
        const keep = { settings:S.settings, lists:S.lists, sound:S.sound, gcal:S.gcal, passport:S.passport };
...
async function fbInit() {
  ...
  FB.auth = {
    ...
    /* Identity only. Calendar scopes are sensitive: attaching them here is
       what makes Google show “this app hasn’t been verified” on every login.
       Calendar is a separate opt-in (gcalAuthorize / GIS) after sign-in. */
...
/* ---------- What's New after sign-in ----------
   Google, email and local profiles all come through enterApp. The card
   names the current version and its notes. It also fires when this
   account has not opened the pack for a week, even if the number is the
   same — so a long gap still gets a catch-up. Dismiss once and it stays
   quiet until the next version (or the next week away). Embeds
   (auth:'none') never show it. */
function whatsNewStorageKey() {
  return whatsNewDeviceKey(CFG.storageKey || 'focusdial.v3', AUTH.user && AUTH.user.id);
}
function deviceWhatsNew() { return readWhatsNewRecord(STORE.web() || (typeof localStorage !== 'undefined' ? localStorage : null), whatsNewStorageKey()); }
function cloudWhatsNew() { return { v: S.meta && S.meta.seenVersion || null, last: Number(S.meta && S.meta.lastVisit) || 0 }; }
function resolvedWhatsNew() {
  return mergeWhatsNewRecord(deviceWhatsNew(), cloudWhatsNew());
}
function persistWhatsNew(patch) {
  const rec = Object.assign({ v: null, last: 0 }, resolvedWhatsNew(), patch || {});
  if (!S.meta || typeof S.meta !== 'object') S.meta = { sample:false, created:Date.now(), notice:null };
  if (rec.v) S.meta.seenVersion = rec.v;
  if (rec.last) S.meta.lastVisit = rec.last;
  writeWhatsNewRecord(STORE.web() || (typeof localStorage !== 'undefined' ? localStorage : null), whatsNewStorageKey(), rec);
  if (AUTH.user) save();
}
function whatsNewLede(payload) {
  ...
  const how = provider === 'google' ? 'Signed in with Google.'
    ...
    : 'You are signed in.';
  if (payload && payload.kind === 'welcome') {
    return how + ' It has been a while — here is this version at a glance.';
  }
  return how + ' Here is what is new.';
}
...
function forceWhatsNew() {
  try { return typeof location !== 'undefined' && /(?:^|[?&])whatsnew=1(?:&|$)/.test(location.search); } catch (e) { return false; }
}
function whatsNewBody(payload) {
  const rels = (payload && payload.releases) || [];
  const kicker = payload && payload.kind === 'welcome' ? '<p class="wn-kicker">Welcome back</p>' : '<p class="wn-kicker">What\u2019s new</p>';
  const list = rels.map(r => `
    <section class="wn-rel">
      <div class="wn-head">
        <span class="wn-badge">v${esc(r.version)}</span>
        <h3>${esc(r.name)}</h3>
        ${r.date ? `<time class="wn-date" datetime="${esc(r.date)}">${esc(fmtRelDate(r.date))}</time>` : ''}
      </div>
      <ul class="wn-list">${(r.highlights || []).map(h => `<li>${esc(h)}</li>`).join('')}</ul>
    </section>`).join('');
  return `${kicker}<p class="wn-lede">${esc(whatsNewLede(payload))}</p>${list}`;
}
function showWhatsNewModal(payload, onDone) {
  const name = payload && payload.name ? payload.name : '';
  const title = payload && payload.kind === 'welcome'
    ? (name ? `Welcome back · ${name}` : `Welcome back · v${payload.version}`)
    : (name ? `${name} · v${payload.version}` : `Version ${payload.version}`);
  modalDone = () => { persistWhatsNew({ v: VERSION, last: Date.now() }); save(); if (onDone) onDone(); };
  openModal(title, whatsNewBody(payload), [
    { label: 'Open Help', onClick: () => { go('help'); } },
    { label: 'Got it', primary: true }
  ]);
  announce((payload && payload.kind === 'welcome' ? 'Welcome back. ' : '') + 'Version ' + VERSION + (payload && payload.name ? ' — ' + payload.name : ''));
}
function maybeShowWhatsNew(after) {
  const skip = () => { persistWhatsNew({ v: VERSION, last: Date.now() }); if (after) after(); return false; };
  if (AUTH.mode === 'none') return skip();
  const scrim = $('#scrim'), title = $('#modalTitle'), body = $('#modalBody'), foot = $('#modalFoot');
  if (!scrim || !title || !body || !foot) return skip();
  const rec = resolvedWhatsNew();
  const payload = forceWhatsNew()
    ? whatsNewPayload({ seenVersion: null, lastVisit: 0, now: Date.now(), awayMs: AWAY_MS, version: VERSION, releases: RELEASES })
    : whatsNewPayload({
        seenVersion: rec.v, lastVisit: rec.last, now: Date.now(),
        awayMs: AWAY_MS, version: VERSION, releases: RELEASES
      });
  persistWhatsNew({ last: Date.now() });
  if (!payload) return skip();
  showWhatsNewModal(payload, after);
  return true;
}
function openWhatsNewNow() {
  const cur = RELEASES.filter(r => cmpVersion(r.version, VERSION) <= 0).slice(0, 2);
  const payload = { kind: 'update', version: VERSION, name: (cur[0] && cur[0].name) || '', releases: cur.length ? cur : RELEASES.slice(0, 1) };
  showWhatsNewModal(payload);
}
...
function applyTheme() {
  ...
  const accent = ACCENT_IDS.includes(S.settings.accent) ? S.settings.accent : 'focus';
  if (S.settings.accent !== accent) S.settings.accent = accent;
  document.documentElement.setAttribute('data-accent', accent);
  const grad = GRADIENT_IDS.includes(S.settings.accentGradient) ? S.settings.accentGradient : 'off';
  if (S.settings.accentGradient !== grad) S.settings.accentGradient = grad;
  if (grad === 'off') document.documentElement.removeAttribute('data-gradient');
  else document.documentElement.setAttribute('data-gradient', grad);
...
function drawDaily(keys) {
  ...
  const capDaily = $('#capDaily');
  if (capDaily) capDaily.textContent = `Completed focus intervals only. Peak in this range: ${vals[maxI]} minutes.`;
}
...
/* =====================================================================
   GOOGLE CALENDAR — progressive sync straight from the browser.
   ...
   Connect asks only to *view* events (calendar.events.readonly) and list
   calendars (calendar.calendarlist.readonly). Write
   (calendar.events) is requested later, when the person turns on
   “Send my blocks to Google”. Never the broad /auth/calendar scope.
   ===================================================================== */
const GCAL_SCOPE = {
...
const GCAL_CONNECT_SCOPES = [GCAL_SCOPE.read, GCAL_SCOPE.list];
...
const GCAL = { state:'idle', cals:[], events:{}, error:null, lastAt:null, lastPushAt:null, busy:false,
               token:null, exp:0, canList:false, canWrite:false, pushing:false, pushT:null, planSig:null };
const enc = encodeURIComponent;
const gcalReady = () => !!CFG.googleClientId;
const gcalTokenValid = () => !!GCAL.token && Date.now() < GCAL.exp - 60000;
const gcalHasScope = (r, scope) => {
  try { return !!(window.google && google.accounts.oauth2.hasGrantedAllScopes(r, scope)); }
...
function gcalLoadToken() {
  try {
    ...
    if (t && t.exp > Date.now() + 60000) {
      GCAL.token = t.token; GCAL.exp = t.exp; GCAL.canList = !!t.canList;
      /* Legacy tokens (before 3.7.3) were always write-capable. */
      GCAL.canWrite = t.canWrite == null ? true : !!t.canWrite;
    }
  } catch (e) {}
}
function gcalStoreToken() {
  try {
    if (GCAL.token) sessionStorage.setItem(GCAL_TOKEN_KEY, JSON.stringify({
      token:GCAL.token, exp:GCAL.exp, canList:GCAL.canList, canWrite:GCAL.canWrite
    }));
...
/* Opens Google's consent popup. Call it straight from a click.
   Pass the scopes this gesture needs. include_granted_scopes keeps
   earlier grants on the new token (GIS incremental auth). */
function gcalAuthorize(scopes) {
  const wanted = (scopes && scopes.length) ? scopes : GCAL_CONNECT_SCOPES;
  return new Promise((ok, no) => {
    ...
    const client = google.accounts.oauth2.initTokenClient({
      ...
      callback: r => {
        ...
        const canRead = gcalHasScope(r, GCAL_SCOPE.read) || gcalHasScope(r, GCAL_SCOPE.write);
        const askedRead = wanted.includes(GCAL_SCOPE.read) || wanted.includes(GCAL_SCOPE.write);
        if (askedRead && !canRead) {
          no(gErr('Calendar access was not granted. Try again and tick the calendar box on Google’s screen.', { code:'scope_denied' })); return;
        }
        if (wanted.includes(GCAL_SCOPE.write) && !gcalHasScope(r, GCAL_SCOPE.write)) {
          no(gErr('Calendar editing was not granted. You can keep viewing events without it.', { code:'write_denied' })); return;
        }
        GCAL.token = r.access_token;
        GCAL.exp = Date.now() + (+r.expires_in || 3600) * 1000;
        if (gcalHasScope(r, GCAL_SCOPE.list)) GCAL.canList = true;
        if (gcalHasScope(r, GCAL_SCOPE.write)) GCAL.canWrite = true;
        gcalStoreToken(); ok();
      },
      error_callback: e => no(gErr(e && e.message || 'The Google window closed', { code:e && e.type }))
    });
    client.requestAccessToken({ scope: wanted.join(' ') });
  });
}
async function gcalEnsureWrite() {
  if (GCAL.canWrite && gcalTokenValid()) return true;
  try {
    await gcalAuthorize([GCAL_SCOPE.write]);
    return !!GCAL.canWrite;
  } catch (e) {
    gcalAuthFailed(e);
    return false;
  }
...
async function gcalPush() {
  if (GCAL.pushing || !S.gcal.on || !S.gcal.push || !GCAL.canWrite || S.meta.sample || !gcalTokenValid()) return null;
...
function gcalQueuePush() {
  if (!S.gcal || !S.gcal.on || !S.gcal.push || !GCAL.canWrite || !gcalTokenValid()) return;
...
async function gcalConnect() {
  try { await gcalAuthorize(GCAL_CONNECT_SCOPES); } catch (e) { gcalAuthFailed(e); return; }
  GCAL.busy = true; renderGcal();
  try {
    ...
    await gcalFetchWeek();
    GCAL.busy = false;
    toast('Google Calendar connected — view only. Turn on Send my blocks if you want events written to Google.');
  } catch (e) { gcalFail(e); }
  GCAL.busy = false;
...
async function gcalSyncNow() {
  ...
  } else if (GCAL.state === 'live') {
    toast('Up to date with Google (view only)');
  }
}
function gcalDisconnect() {
  ...
  S.gcal.on = false; GCAL.events = {}; GCAL.cals = []; GCAL.error = null; GCAL.state = 'idle'; GCAL.canList = false; GCAL.canWrite = false; save();
...
async function gcalRemoveAll() {
  if (!(GCAL.canWrite && gcalTokenValid())) {
    if (!(await gcalEnsureWrite())) return;
  }
  let n = 0;
  try {
...
function gcalReset() {
  ...
  Object.assign(GCAL, { state:'idle', cals:[], events:{}, error:null, lastAt:null, lastPushAt:null, busy:false, planSig:null, canList:false, canWrite:false });
}
...
function renderGcal() {
  ...
  const head = '<div class="panel-head"><h3>Google Calendar</h3>' +
    (S.gcal.on ? `<button class="btn sm ghost" id="gSync" ${GCAL.busy || GCAL.pushing ? 'disabled' : ''}>${GCAL.busy || GCAL.pushing ? 'Syncing…' : 'Sync now'}</button>`
               : '<span class="eyebrow">view first</span>') + '</div>';
  let body = '';
  if (!gcalReady()) {
    ...
    body = `<p style="font-size:calc(12px*var(--ts,1));color:var(--muted);margin:0 0 10px">Optional. Sign-in never asks for this. Connect only <strong>views</strong> your events so the planner can work around them. Sending blocks to Google is a later step you can refuse.</p>
      ...
      <p style="font-size:calc(11px*var(--ts,1));color:var(--muted);line-height:1.5;margin:9px 0 0">Google will ask to view your calendars and events — not to change them. Those permissions are still sensitive, so until Google finishes reviewing the app the popup may say <em>unverified</em>. You can decline and keep using the pack. Access lasts about an hour and is never stored on a server. <a href="privacy.html">Privacy</a>.</p>`;
  } else {
    ...
    body += `<div class="switch-row" style="padding:6px 0"><div><div class="lbl" style="font-size:calc(12.5px*var(--ts,1))">Send my blocks to Google</div>
        <div class="hint">${GCAL.canWrite ? 'New, edited and deleted blocks follow automatically. Blocks imported from .ics stay here.' : 'Asks Google for edit access, then writes only events this pack created. You can refuse and keep viewing.'}</div></div>
        <input type="checkbox" id="gPush" ${S.gcal.push && GCAL.canWrite ? 'checked' : ''}></div>`;
  ...
  const ps = $('#gPush'); if (ps) ps.onchange = async e => {
    if (!e.target.checked) { S.gcal.push = false; GCAL.planSig = null; save(); return; }
    e.target.disabled = true;
    const ok = await gcalEnsureWrite();
    e.target.disabled = false;
    if (!ok) { e.target.checked = false; S.gcal.push = false; save(); renderGcal(); return; }
    S.gcal.push = true; GCAL.planSig = null; save();
    gcalPush();
    gcalSyncAllHabits();
  };
...
function gcalAfterLoad() {
  ...
  loadGis().catch(() => {});
  gcalLoadToken();
  if (S.gcal.on && gcalTokenValid()) gcalRefresh().then(() => { if (S.gcal.push && GCAL.canWrite) gcalPush(); });
  else renderGcal();
}
function gcalBoot() {
  if (!gcalReady()) { renderGcal(); return; }
...
const HIDEABLE_VIEWS = [['plan','Plan'],['matrix','Matrix'],['notes','Notes'],['sound','Sound'],['calm','Calm'],['mood','Mood'],['habits','Habits'],['stats','Stats'],['passport','Passport'],['help','Help'],['about','About']];
...
function accentPickerHtml() {
  const cur = ACCENT_IDS.includes(S.settings.accent) ? S.settings.accent : 'focus';
  return `<div class="set-row" style="align-items:flex-start"><div class="set-text"><span class="set-label">Accent colour</span>
      <p class="set-hint" id="accent_h">Classic study hues plus calmer professional options. Preview updates at once.</p></div>
    <div class="set-control" style="flex-direction:column;align-items:flex-start;gap:6px">
      <div class="accent-grid" role="radiogroup" aria-label="Accent colour" aria-describedby="accent_h">${ACCENT_PALETTE.map(a =>
        `<button type="button" class="accent-swatch" role="radio" data-st="accent" data-val="${a.id}" aria-checked="${a.id === cur}" aria-label="${esc(a.name)}" style="--sw:${a.sw}" title="${esc(a.name)}"></button>`
      ).join('')}</div>
      <div class="accent-legend">${ACCENT_PALETTE.map(a =>
        `<span><i style="--sw:${a.sw}"></i>${esc(a.name)}</span>`).join('')}</div>
    </div></div>`;
}
function renderSeeingPanel() {
  $('#set-seeing').innerHTML = panelHead('Seeing', 'Size, contrast and colour. Changes apply as you make them — the preview at the bottom shows the result.')
    ...
    + group('Colour', accentPickerHtml()
      + rowChoice(['st','accentGradient'], 'Accent gradient', 'Optional wash behind the app and gradient fills on primary buttons. Off keeps solid colour — best if motion or colour sensitivity is high.', [['off','Off'],['soft','Soft'],['rich','Rich']]))
    + advanced('seeing', rowChoice(['cf','spacing'], 'Line and letter spacing', 'More space between lines and letters helps many readers, including people with dyslexia.', [['normal','Normal'],['relaxed','Relaxed'],['loose','Loose']])
      ...
      { label:'Typeface, intensity and focus', lead:'Spacing, the reading typeface, how strong the colour is, and how the keyboard outline looks.' })
    + `<div class="set-preview" aria-hidden="true"><div class="eyebrow">Preview</div>
        ...
        <div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:10px"><span class="chip"><span class="dot" style="background:var(--accent)"></span>45 min today</span>
...
function setSetting(key, value) {
  ...
  if (key === 'theme' || key === 'accent' || key === 'accentGradient') applyTheme();
  save();
  ...
  if (key === 'accent') {
    const name = (ACCENT_PALETTE.find(a => a.id === value) || {}).name || value;
    announce('Accent: ' + name);
  }
  if (key === 'accentGradient') announce('Gradient: ' + value);
  renderSettings();
}
...
$('#view-settings').addEventListener('click', e => {
  const sw = e.target.closest && e.target.closest('.accent-swatch[data-st]');
  if (sw) {
    setSetting(sw.dataset.st, sw.dataset.val);
    return;
  }
...
async function gcalHabitSync(h) {
  if (!h || !S.gcal.on || !gcalTokenValid() || !GCAL.canWrite || !h.remindAt || S.meta.sample) return;
  try {
    const cal = S.gcal.target || 'primary';
...
async function gcalSyncAllHabits() {
  if (!S.gcal.on || !gcalTokenValid() || !GCAL.canWrite) return;
...
const HELP_DOCS = [
  ...
  { id:'passport', title:'Passport and achievements', category:'Wellness', view:'passport', viewLabel:'Passport',
    body:'Your card in this workspace: a display name, a photo kept as part of the workspace (not a second login app), and badges that unlock from focus sessions you actually finish. First Spark, Hour Hero, streaks and Focus Legend all come from the same history as Statistics.' },
  ...
  { id:'whatsnew', title:'What’s new in this version', category:'Sync', view:'settings', viewLabel:'Setup',
    body:'After you sign in with Google, email, or a local profile, a short card names the current version and what changed. If you have not opened the pack for a week, the same card welcomes you back. Dismiss it once and it stays quiet until the next version. You can open it again from About, at the version line.' },
  { id:'sync', title:'Cloud and Google Calendar sync', category:'Sync', view:'plan', viewLabel:'Plan',
    body:'Signed out, everything stays in this browser. Signed in with Google, only your name and email are requested — workspace and history sync to your own Firestore space. Google Calendar is a separate button on Plan: it is not part of sign-in. Calendar access lasts about an hour at a time — press Sync now to refresh. Until Google verifies the calendar permission, that second popup may say the app is unverified; you can skip it and still use the pack. To download an .ics manually: Google Calendar → Settings → Settings for my calendars → your calendar → Integrate calendar.' },
...
const AB_NOTES = [
  ['Where the data is', 'Signed out, everything is in this browser: the workspace in localStorage, your history in an IndexedDB database. Signed in, both go to your own Firestore space under rules that let nobody else read them. Google sign-in only receives your name and email. Calendar is optional and never requested at login. No analytics, nothing sold. Full detail is on the Privacy page.'],
...
function renderAbout() {
  ...
  $('#abNotes').innerHTML = `<h3 class="ab-h">Straight answers</h3>` + AB_NOTES.map(([t, p]) =>
    ...
    `<p class="ab-version">Version ${VERSION} · <button type="button" class="linkish" id="abWhatsNew">What’s new</button> · <a href="privacy.html">privacy</a> · <a href="${esc(CFG.projectUrl || CFG.projectsUrl || '#')}">how it was built</a> · <a href="${esc(CFG.homeUrl || '#')}">the rest of the site</a></p>`;
  const wn = $('#abWhatsNew'); if (wn) wn.onclick = openWhatsNewNow;
  abAnimate();
}
...
function renderWho() {
  ...
  const u = AUTH.user;
  const name = (S.passport && S.passport.displayName) || u.name || u.email || 'You';
  const av = S.passport && S.passport.avatar;
...
async function enterApp(user) {
  ...
  setTimeout(() => maybeShowWhatsNew(afterSignIn), 280);
}
...
/* =====================================================================
   PASSPORT — profile card + achievement unlocks from S.sessions
   ===================================================================== */
let unlockQueue = [];
function seedPassportFromAuth() {
  S.passport = normalisePassport(S.passport);
  if (!S.passport.displayName && AUTH.user) {
    S.passport.displayName = String(AUTH.user.name || (AUTH.user.email || '').split('@')[0] || '').slice(0, 80);
  }
}
function afterHistoryLoaded() {
  seedPassportFromAuth();
  const stats = passportStats(S.sessions || []);
  const next = adoptKeys(S.passport, stats, { silent:true });
  S.passport = next.passport;
}
function revealAchievements() {
  seedPassportFromAuth();
  const stats = passportStats(S.sessions || []);
  const next = adoptKeys(S.passport, stats);
  S.passport = next.passport;
  if (next.fresh.length) { save(); showUnlocks(next.fresh); }
  if (view === 'passport') renderPassport();
  renderWho();
}
function showUnlocks(keys) {
  unlockQueue = (keys || []).map(getAchievement).filter(Boolean);
  if (!unlockQueue.length) return;
  paintUnlock();
}
function paintUnlock() {
  const overlay = $('#unlockScrim'); if (!overlay) return;
  const a = unlockQueue[0];
  if (!a) { overlay.hidden = true; return; }
  overlay.hidden = false;
  $('#unlockIcon').textContent = a.icon;
  $('#unlockTitle').textContent = a.title;
  $('#unlockDesc').textContent = a.description;
  $('#unlockNext').textContent = unlockQueue.length > 1 ? `Next (1 of ${unlockQueue.length})` : 'Keep going';
  announce('Achievement unlocked: ' + a.title);
  const btn = $('#unlockNext'); if (btn) btn.focus();
}
function advanceUnlock() {
  unlockQueue.shift();
  if (unlockQueue.length) paintUnlock();
  else { const o = $('#unlockScrim'); if (o) o.hidden = true; }
}
function passportPhotoMarkup() {
  const name = (S.passport && S.passport.displayName) || (AUTH.user && (AUTH.user.name || AUTH.user.email)) || '';
  if (S.passport && S.passport.avatar) return `<img src="${esc(S.passport.avatar)}" alt="">`;
  return esc(passportInitials(name, '?'));
}
function renderPassport() {
  if (!$('#view-passport')) return;
  seedPassportFromAuth();
  const stats = passportStats(S.sessions || []);
  const earned = earnedKeys(stats);
  const photo = $('#passportPhotoBtn');
  if (photo) photo.innerHTML = passportPhotoMarkup();
  const email = $('#passportEmail');
  if (email) email.textContent = (AUTH.user && AUTH.user.email) || (AUTH.user ? 'Local profile on this device' : 'Workspace on this browser');
  const nameEl = $('#passportName'), bioEl = $('#passportBio');
  if (nameEl && document.activeElement !== nameEl) nameEl.value = S.passport.displayName;
  if (bioEl && document.activeElement !== bioEl) bioEl.value = S.passport.bio;
  const count = $('#passportBadgeCount');
  if (count) count.textContent = `${earned.length} of ${ACHIEVEMENTS.length}`;
  const box = $('#passportBadges');
  if (box) box.innerHTML = ACHIEVEMENTS.map(a => {
    const p = achievementProgress(a, stats);
    const on = p >= 1;
    return `<article class="p-badge ${on ? 'on' : ''}">
      <span class="p-ico" aria-hidden="true">${esc(a.icon)}</span>
      <h4>${esc(a.title)}</h4>
      <p>${esc(a.description)}</p>
      <div class="p-bar" role="progressbar" aria-valuemin="0" aria-valuemax="100" aria-valuenow="${Math.round(p * 100)}" aria-label="${esc(a.title)} progress"><span style="width:${Math.round(p * 100)}%"></span></div>
    </article>`;
  }).join('');
  const sc = $('#passportStatsCard');
  if (sc) sc.innerHTML = `<div class="panel-head"><h3>From real sessions</h3></div>
    <div class="ab-stats" style="grid-template-columns:1fr;gap:10px">
      <div class="ab-stat"><strong>${stats.sessions}</strong><span class="ab-stat-k">blocks finished</span></div>
      <div class="ab-stat"><strong>${Math.round(stats.minutes)}</strong><span class="ab-stat-k">focused minutes</span></div>
      <div class="ab-stat"><strong>${stats.streak}</strong><span class="ab-stat-k">day streak</span></div>
    </div>`;
...
function readAvatarFile(file) {
  return new Promise((resolve, reject) => {
    if (!file || !String(file.type || '').startsWith('image/')) return reject(new Error('Please pick an image file'));
...
function wirePassport() {
  const chip = $('#whoChip');
  if (chip) chip.addEventListener('click', () => go('passport'));
  const next = $('#unlockNext');
  if (next) next.addEventListener('click', advanceUnlock);
  const overlay = $('#unlockScrim');
  if (overlay) overlay.addEventListener('click', e => { if (e.target.id === 'unlockScrim') advanceUnlock(); });
  const pick = $('#passportPhotoPick');
  const file = $('#passportPhoto');
  ...
  if (file) file.addEventListener('change', async ev => {
    ...
    if (!f) return;
    try {
      S.passport.avatar = await readAvatarFile(f);
      save(); renderPassport(); renderWho();
      toast('Photo updated');
    } catch (err) { toast((err && err.message) || 'Could not use that image', 0, 'alert'); }
  });
  const nameEl = $('#passportName');
  if (nameEl) nameEl.addEventListener('input', () => {
    S.passport.displayName = nameEl.value.slice(0, 80);
    save(); renderWho();
  });
  const bioEl = $('#passportBio');
  if (bioEl) bioEl.addEventListener('input', () => {
    S.passport.bio = bioEl.value.slice(0, 280);
    save();
  });
}
...
function renderAll() {
  ...
  renderCalendar(); renderBoard(); renderSound(); renderCalm(); renderMood(); renderHabits(); renderPassport(); renderSettings(); renderStats(); renderGcal();
...
(async function init() {
  renderTicks(); applyTheme(); renderSiteNav(); applyBrand(); wireGate(); wirePassport();
